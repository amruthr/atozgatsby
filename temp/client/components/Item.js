        ...
        <Container style={{paddingTop:'5px', paddingBottom:'30px',width:'100%', cursor: 'pointer'}}>
          <Row style ={isMobile?{background:'white', padding:'5px'}:{background:'white', margin:'10px 10px', paddingTop:'10px'}}>
           ...
           {infoItem.qty> 0?
            ...
            {  infoItem.rental ==0 || infoItem.rental == undefined ?"":
              <div><p>Available for rent starting at ₹{infoItem.rental} for 3 days</p> <Button className="bg-dark text-white" onClick={()=>window.location.href=`https://api.whatsapp.com/send?phone=918095958811&text=I%20want%20to%20rent%20this%20Item%20-%20${infoItem.title}%0A%0A${window.location.href}`}>Rent Now</Button><small class="d-block my-2">We currently Accept rentals in Mangalore & Udupi</small></div>
            }          
          ...
          {isMobile &&  <Button onClick ={()=>{navigator.share?        
            navigator.share({
              title:'Vastram Fashion- '+infoItem.title ,
              text:"Check out "+infoItem.title+" from Vastram !",
