...
dataload.map((item)=>{
  ...
  componentWillReceiveProps(nextProps) {
        ...
        <Col md="3" style={styles.card} key={x.imgSrc}>
            ...
            <div className="shadow mx-md-4 mt-3 " style={styles.cardBtn} onClick={()=>oneKeywordForFilter(x)}>
              <Link to={`/productslist/${x.cardTitle}/`}>
                <CardImg  loading="lazy"  src={x.imgSrc} alt={x.cardTitle}  />
                  <b> {x.cardTitle} </b>
              </Link>
            </div>  
    ...
    backgroundSize: 'contain',}}>
      <h5 className="mx-4 mx-md-5 my-3 text-white" style={{ fontFamily:'Open Sans'}}><Link to="/" style={{color:'#fff',}}><FiHome/></Link> › {this.state.catname} Collections</h5>
