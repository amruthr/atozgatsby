...
import {FaCheck, FaSpinner, FaWindowClose} from 'react-icons/fa'
...
.catch((err)=>console.log(err))   
  ...
  onSubmit = (title, price, color, size, qty , tags, images, description, sizechart, sizedetails, special, brands, rental ) => {
  ...
  onChangeRental = (e) => this.setState({rental: e.target.value})
  ...
  render() {
   ...
   // })
    ...
    const { title, price, color, size, qty, tags, images, description, sizechart, sizedetails, special, brands, rental  } = this.state
    const style = {
      height: '100px',
      display: 'flex',
      flexDirection: 'row',
    ...
    return (
      ...
      <Form >
        ...
        <FormGroup>
          ...
          <Button  onClick={this.onChangeSpecial}>{this.state.special?<FaCheck/>:<FaWindowClose/>}</Button>
        </FormGroup>
        <FormGroup>
        ...
        <FormGroup>
          ...
          <FormGroup>
          <Label for="exampleEmail">Rental<small>(Enter price for 3 day rental keep 0 if not for rental)</small></Label>
          <Input type = "number" placeholder='Rental Price'  onChange={this.onChangeRental} />
          </FormGroup> 
      <FormGroup>
 ...
 <div style={style}>      
      ...
      <Button onClick={()=>this.onSubmit(
        ...
        tags,
        images,
        description, sizechart, sizedetails, special, brands , rental
        )}>Submit<FaSpinner className="spin" style={{display:this.state.loading?'block':'none'}}/></Button>
      </Container>
    );
  }
}
