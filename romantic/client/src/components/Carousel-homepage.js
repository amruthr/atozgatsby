    ...
    const slides = items.map(x => {
      return (
       <CarouselItem 
          ...
          link={x.title}
        > <Link to={x.title}>
          <Row  style={isMobile?{backgroundColor: '#fff', height:'30vh', marginTop:'0vh'}:{backgroundColor: '#fff', padding:'3px', height:'62vh', marginBottom:'0px'}}>
            <Col md={isMobile?"6":"12"}>
              <img loading="lazy" src={x.src}  alt={x.altText} style={{width: '100%', height: '-webkit-fill-available', borderRadius:'15px'}}/>
            </Col>
