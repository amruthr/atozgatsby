...
window.history.pushState("Dashboard", "Vastram", tab)
}
  ...
  render() {
    ...
    return (
      <div style={{display:'flex', flexDirection:isMobile?'column':'row', alignItems:'center'}}>
        ...
        <NavItem >
            <NavLink  style={styles.tabx} onClick={() => { this.toggle('0'); }}>
              <b>Dashboard</b>
            </NavLink> 
        </NavItem>
          <NavItem >
        ...
        <MobileOnlyView className='text-center mt-2 '>
          {this.state.activeTab != '0' && <Button onClick={() => { this.toggle('0'); }}>Back to Dashboard</Button>}
        </MobileOnlyView>
      </div>
    )
  }
