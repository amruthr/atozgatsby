        ...
        <div style={{ bottom: '20px', position:'absolute'}}> 
         {x.subcats.map((item, ind)=>
             <Link to={`/productslist/${item}`}>
               <small  style={{color:'#000', background:'#fff', borderRadius:'20px' ,  padding: '3px 10px',margin :'5px', border:'solid 0.5px lightgrey',}}> {item}</small>
             </Link>
          )}     
        </div>          
      </Link> 
               ...
               :
    <div key={x._id} className="animated fadeIn" style={{backgroundImage:'url('+x.images[0]+')', backgroundRepeat: 'round', height:'250px', width:isMobile?'100%':'31%', borderRadius:'10px', margin:'10px', boxShadow:'-2px 2px 10px 0px lightgrey', padding:'30px 15px', position:'relative'}}>
        <div>              
           {isAdmin ?  
               <div> 
                  <Link to={`/category/${x._id}`} style={{color:'#000', textDecoration:'none'}}>  
                      <h5 style={{background:'#fff', borderRadius:'20px',width: 'fit-content',  padding: '3px 10px'}}>{x.catname}{x.imagedata}</h5>
                  </Link>                  
                <EditCategory infos={x} />
                {x.gender=="male"? <span className="bg-primary text-white rounded p-2"><FaMale/> </span> : <span className="bg-warning text-white rounded p-2"><FaFemale/></span> }
