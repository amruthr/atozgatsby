...
class Headstyle extends Component{
    ...
    render(){
        const style = { fontFamily:'Jost', letterSpacing:'3px', margin:'2px 5px', fontSize: 'large', fontWeight:'bolder', padding:'5px', textAlign:'left', color:this.props.color || '#000'};
        const {headtext}= this.props;
        return(
       <div className="text-uppercase wow animated fadeIn d-flex  my-4">    
            <p  className="d-inline ml-3" style={style}>{headtext}</p> 
       </div>
        );
    }
