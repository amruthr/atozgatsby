...
sendMail = (ref)=>{
  ...
  axios.post('/mail/mail',{
    ...
    subject: "Vastram Fashion - Order Recieved from "+this.state.firstName,
    html:  `<h3>Dear ${this.state.firstName}, thank you for shopping with us. </h3>
    <h4>this is a self generated e-bill. for any queries call us at 8095958811</h4><hr>
    ...
    <hr>
    <a href="https://vastramfashions.com/order/${ref}"> Check order status</a>`})
  .catch(err => console.log(err))
    ...
    openCheckout(data) {
      const { getCart, selectorTotalAmountCart } = this.props;
      const {totalDelivery} = this.state;
      let options = {
        "key": "rzp_test_3vcTRAChgc79JC",
        "amount": (selectorTotalAmountCart+totalDelivery)*100, // 2000 paise = INR 20, amount in paisa
        "name": "Vastram fashion",
        "description": "Vastram Purchase",
        "image": "/images/vlogo.jpg",
        "handler": function (response){
          axios.post('/api/confirmpayment',{
          payment_id :response.razorpay_payment_id,
          order_id:data,
          secret:response.razorpay_signature
          }).then(window.location.href='/order/'+data);
        },
        "order_id": data,
        "prefill": {
  ...
  onSubmitOrder = () => {
    ...
    axios.post('/api/add/orders', {
      ...
      this.sendMail();
      this.openCheckout(data.data.id);
    })
    .catch(error => {
      console.log(error);
    });
