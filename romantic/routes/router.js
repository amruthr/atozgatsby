...
router.post('/update/item', (req, res, next)=> {
  const { id, title, price, color, size, tags, qty, images, description, sizechart, sizedetails, special, brands } = req.body
  ModelProducts.findById(id, (err, model) => {
    if (err) return console.log(err);
  
    model.set({ title, price, color, size, tags, qty, images, description, sizechart, sizedetails, special, brands  });
    model.save(function (err, updatedItem) {
      if (err) return console.log(err);
    });
  }).then(()=>{
...
router.post('/add/item', (req, res, next)=> {
  const { title, price, description, size, tags, images, color ,qty, sizechart,sizedetails,special, brands, rental } = req.body
...
router.post('/add/orders', (req, res, next)=> {
  ...
  instance.orders.create({amount:parseInt(totalAmount*100), currency:'INR', receipt:'trabn_111', payment_capture:1, notes:''}, )
  .then((data)=>{
...
router.post('/confirmpayment', (req, res)=>{
  ...
  ModelOrders.findOne({ref:order_id},(err, x)=>{
    err ? res.status(500).send(err) :
    x.set({payment:payment_id})
    x.save(function (err, updatedItem) {
  if (err) return res.status(301)
  else {
      axios.post(`https://${config.RZPKEY}:${config.RZPSECRET}@api.razorpay.com/v1/payments/${payment_id}/capture`,
      {
        amount: updatedItem.totalAmount*100,
        currency: 'INR'
      })
      .catch(err => com=nsole.log(err))
...
router.post('/update/category', (req, res, next)=> {
  const { id, catname, images, subcats, hide, gender} = req.body
  ModelCategories.findById(id, (err, model) => {
    if (err) return handleError(err);
  
    model.set({ catname, images, subcats, hide, gender});
    model.save(function (err, updatedItem) {
      if (err) return console.log(err);
